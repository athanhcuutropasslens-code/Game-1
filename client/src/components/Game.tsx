...
import { calculateItemCost, findMergeTarget, getMergeableItems, getMergedItemStats, mergeItems, updateItemDerivedData } from '@/lib/game/merge';
...
function Game() {
  ...
  const enterRoom = useCallback((r) => {
    ...
    if (['COMBAT', 'ELITE', 'BOSS'].includes(r.type)) { setMonster(generateMonster(floor, r.type, zone, randomId, applyStatusEffect)); setDiceResult(null); setGameState(GAME_STATE.COMBAT); }
  ...
  const handleMerge = useCallback((item1) => {
    ...
    const matchItem = findMergeTarget(item1, player.inventory, player.equipment);
    if (!matchItem) return addLog(item1.type === 'CONSUMABLE' ? 'Không đủ bản sao consumable cùng độ hiếm để nâng cấp!' : 'Không có trang bị trùng id và level để hợp nhất!', 'text-red-500');
    ...
    const equippedSlot = slot && Object.entries(player.equipment).find(([, equippedItem]) => equippedItem?.uid === matchItem.uid)?.[0];
    const shouldEquip = slot && ((item1.isEquipped || false) || !!equippedSlot);
    if (equippedSlot) newEquip[equippedSlot] = null;
    if (slot && shouldEquip) newEquip[slot] = merged; else newInv.push(merged);
    setPlayer((p) => ({ ...p, inventory: newInv, equipment: newEquip })); setSelectionUID(merged.uid); addLog(item1.type === 'CONSUMABLE' ? `Nâng cấp consumable thành công: ${merged.name} (R${merged.rarity})` : `Hợp nhất thành công: ${merged.name} +${merged.level}`, 'text-purple-400'); addEffect('MERGE!', 'heal', 50, 50);
                ...
                <div className="bg-slate-800 p-4 border-t border-slate-700">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    {(selectedItem.type !== 'SERVICE') && (() => {
                      const mergeCandidates = getMergeableItems(selectedItem, player.inventory, player.equipment);
                      const canMerge = mergeCandidates.length > 0;
                      const isConsumable = selectedItem.type === 'CONSUMABLE';
                      return (
                        <button 
                          onClick={() => canMerge && handleMerge(selectedItem)}
                          disabled={!canMerge}
