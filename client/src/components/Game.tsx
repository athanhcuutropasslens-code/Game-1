...
const calculateCost = calculateItemCost;

const getItemStats = getMergedItemStats;
...
const generateItem = (baseItem, level = 1, forceRarity = null) => {
  ...
  return updateItemDerivedData(item);
};
...
function Game() {
        ...
        monsterDamage = Math.max(
          ...
          Math.floor(
            (Math.floor(Math.random() * monster.diceSides) +
              ...
              defendMult *
              (1 +
                ((nextPlayer.effects || []).some((e) => e.incomingPercent)
                  ? 0.2
                  : 0)),
          ),
        );
  ...
  const handleMerge = useCallback(
    (item1) => {
        ...
        item1.type === "WEAPON"
          ...
          : item1.type === "ARMOR"
            ...
            : item1.type === "ACCESSORY"
              ? "accessory"
              : null;
      const matchItem = findMergeTarget(
        item1,
        player.inventory,
        player.equipment,
      );
      ...
      if (!matchItem)
        return addLog(
          item1.type === "CONSUMABLE"
            ? "Không đủ bản sao consumable cùng độ hiếm để nâng cấp!"
            : "Không có trang bị trùng id và level để hợp nhất!",
          "text-red-500",
        );

      const merged = mergeItems(item1, matchItem, randomId);
      ...
      const equippedSlot =
        slot &&
        Object.entries(player.equipment).find(
          ([, equippedItem]) => equippedItem?.uid === matchItem.uid,
        )?.[0];
      const shouldEquip = slot && (item1.isEquipped || !!equippedSlot);

      if (equippedSlot) newEquip[equippedSlot] = null;
      ...
      addLog(
        item1.type === "CONSUMABLE"
          ? `Nâng cấp consumable thành công: ${merged.name} (R${merged.rarity})`
                ...
                <div className="bg-slate-800 p-4 border-t border-slate-700">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    {selectedItem.type !== "SERVICE" &&
                      (() => {
                        const mergeCandidates = getMergeableItems(
                          selectedItem,
                          player.inventory,
                          player.equipment,
                        );
                        ...
                        return (
                          <button
                            onClick={() =>
                              canMerge && handleMerge(selectedItem)
                            }
                            disabled={!canMerge}
                          ...
                          >
                            {isConsumable ? "Nâng Cấp" : "Hợp Nhất"}
                          </button>
                        );
                      })()}
