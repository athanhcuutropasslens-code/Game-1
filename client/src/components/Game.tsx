...
import { generateMonster as generateMonsterEngine } from "@/lib/game/combat";
import {
  absorbShieldDamage,
  applyStatusEffect,
  clamp,
...
import {
  generateLoot as generateLootEngine,
  isItemUsableByClass,
  rollRarity,
  generateItem as generateItemEngine,
} from "@/lib/game/loot";
import { computeFullStats, claimLootRewards } from "@/lib/game/progression";
import {
  buyService as buyServiceEngine,
  generateShop as generateShopEngine,
} from "@/lib/game/shop";
...
const computePlayerStats = (player, tempAlloc = null) =>
  computeFullStats(player, { tempAlloc, classesDb: CLASSES_DB, getItemStats });

const generateGameItem = (baseItem, level = 1, forceRarity = null) =>
  generateItemEngine({
    baseItem,
    level,
    forceRarity,
...
const generateLoot = (zoneId, floor, roomType, playerClass, luck = 0) =>
  generateLootEngine({
    ...
    luck,
    itemsDb: ITEMS_DB,
    classesDb: CLASSES_DB,
    generateItem: (baseItem, level, forceRarity) =>
...
const generateShop = (floor, playerClass) =>
  generateShopEngine({
    ...
    if (item.id === "srv_heal") return { ...item, icon: Syringe };
    if (item.id === "srv_box") return { ...item, icon: Package };
    return item;
  });

const generateMonster = (floor, roomType, zoneData) =>
  generateMonsterEngine({
    floor,
    roomType,
    zoneData,
    monsterPrefixes: MONSTER_PREFIXES,
    monsterTypes: MONSTER_TYPES,
    monsterTypeMap: MONSTER_TYPE_MAP,
    applyStatusEffect: (entity, effectKey, duration, overrideVal = null) =>
      applyStatusEffect(
        entity,
        effectKey,
        duration,
...
function Game() {
  ...
  const { final: stats, breakdown: statBreakdown } = useMemo(
    () => computePlayerStats(player, tempStats),
    [player, tempStats],
  );
  ...
  const selectClass = useCallback(
    (cId) => {
      ...
      let nextPlayer = {
        ...
        skillPoints: 1,
        inventory: [
          generateGameItem(weaponBase, 1),
          generateGameItem(armorBase, 1),
          generateGameItem(ITEMS_DB[0], 1),
          generateGameItem(ITEMS_DB[0], 1),
        ],
        effects: [],
        maxFloor: 1,
  ...
  const handleCombat = useCallback(
    (moveId) => {
      ...
      if (!playerTick.isStunned) {
        ...
        if (monsterDamage > 0) {
          const shieldResult = absorbShieldDamage(
            nextPlayer,
            monsterDamage,
            EFFECTS_DB.SHIELD.id,
          );
          if (shieldResult.damageBlocked > 0)
            addEffect(`BLOCK ${shieldResult.damageBlocked}`, "heal", 68, 70);
          monsterDamage = shieldResult.remainingDamage;
          nextPlayer = shieldResult.entity;
        }
        nextPlayer = {
          ...nextPlayer,
  ...
  const claimLoot = useCallback(() => {
    if (!loot) return;
    const result = claimLootRewards({
      loot,
      player,
      currentStats: stats,
      computeStats: (nextPlayer) => computePlayerStats(nextPlayer),
    });
  ...
  const commitStats = useCallback(() => {
    setPlayer((p) => ({
      ...
      hp: Math.min(
        p.hp,
        computePlayerStats({ ...p, statsAllocated: tempStats }).final.maxHp,
      ),
    }));
    setActiveModal(MODAL_STATE.NONE);
  ...
  const equipItem = useCallback(
    (item) => {
      ...
      if (!isItemUsableByClass(item, player.classId, CLASSES_DB))
        return addLog(
  ...
  const handleMerge = useCallback(
    (item1) => {
      const result = mergeItems({
        ...
        generateItem: (baseItem, level, forceRarity) =>
          generateGameItem(baseItem, level, forceRarity),
        calculateItemCost: calculateCost,
      });
      if (!result.ok)
      ...
      setSelectionUID(result.merged.uid);
      addLog(
        `Hợp nhất thành công: ${result.merged.name} +${result.merged.level}`,
        "text-purple-400",
      );
  ...
  const buyService = useCallback(
    (service) => {
      const result = buyServiceEngine({
        service,
        player,
        currentStats: currentRealStats,
        floor,
        itemsDb: ITEMS_DB,
        classesDb: CLASSES_DB,
        isItemUsableByClass,
    ...
    [player, currentRealStats, floor, addLog],
  );
