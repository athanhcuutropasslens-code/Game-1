...
function Game() {
  ...
  const runTickEffects = useCallback((entity, isPlayer = false, maxHpOverride = null) => {
    ...
    const { entity: processed, isStunned, deltas } = runTickEffectsPure({ entity, maxHp });
    deltas.forEach(({ amount }) => {
      if (amount !== 0) addEffect(`${amount > 0 ? '+' : ''}${amount}`, amount > 0 ? 'heal' : 'damage', isPlayer ? 68 : 32, isPlayer ? 78 : 24);
    });
  ...
  const handleCombat = useCallback((moveId) => {
    ...
    const resolveMonsterAttack = (baseDamage) => {
      ...
      nextPlayer = { ...mitigated.entity, hp: Math.max(0, mitigated.entity.hp - mitigated.damageTaken) };
      if (mitigated.absorbed > 0) addEffect(`BLOCK ${mitigated.absorbed}`, 'heal', 68, 70);
    ...
    if (!playerTick.isStunned) {
      ...
      const baseMonsterDamage = Math.max(0, Math.floor((Math.floor(Math.random() * monster.diceSides) + 1 + monster.atk - currentRealStats.def) * defendMult));
      ...
      const baseMonsterDamage = Math.max(0, Math.floor(Math.random() * monster.diceSides) + 1 + monster.atk - currentRealStats.def);
      monsterDamage = resolveMonsterAttack(baseMonsterDamage);
    }
  ...
  }, [monster, gameState, runTickEffects, player, currentRealStats, addLog, addEffect, zone.id, floor]);
