 ...
 * Design: Retro Arcade Cyberpunk with Pixel Art
 * 
 * Features:
 * - 4 character classes with unique abilities
 * - 4 zones with different difficulties
 * - Turn-based combat with Rock-Paper-Scissors mechanics
 * - Item system with rarity tiers and affixes
 * - Skill progression and stat allocation
 * - Procedural map generation with multiple room types
 */
...
import { 
  ...
  Store, Hammer, Zap, Shirt, Sparkles, Gift, Lock, Minus, Plus, ChevronRight, ChevronLeft, Flame, Droplets, Snowflake, Hourglass, Star, Scroll, Cross, Book, Crown, Wand, Package, Syringe, Search, Info, MapPin
} from 'lucide-react';
...
const CLASSES_DB = {
  warrior: {
    ...
    baseMod: { atk: 2, def: 2, maxHp: 30, luck: 0, crit: 0 },
  ...
  mage: {
    ...
    baseMod: { atk: 8, def: -1, maxHp: -20, luck: 2, crit: 0 },
  ...
  cleric: {
    ...
    baseMod: { atk: 0, def: 3, maxHp: 40, luck: 0, crit: 0 },
...
const EFFECTS_DB = {
  STRONG_ATK: { id: 'eff_strong', name: 'Tăng Lực', type: 'BUFF', icon: Sword, color: 'text-red-400', desc: '+5 ATK', mods: { atk: 5 }, duration: 0 },
  REGEN: { id: 'eff_regen', name: 'Hồi Phục', type: 'BUFF', icon: Heart, color: 'text-pink-400', desc: '+3 HP/lượt', dot: { type: 'HP_FLAT', val: 3 }, duration: 0 },
  POISON: { id: 'eff_poison', name: 'Trúng Độc', type: 'DEBUFF', icon: Droplets, color: 'text-green-400', desc: '-4 HP/lượt', dot: { type: 'HP_FLAT', val: -4 }, duration: 0 },
  SHIELD: { id: 'eff_shield', name: 'Lá Chắn', type: 'BUFF', icon: Shield, color: 'text-blue-300', desc: 'Hấp thụ 20 ST', shieldVal: 20, duration: 0 },
  STUN: { id: 'eff_stun', name: 'Choáng', type: 'DEBUFF', icon: StarsIcon, color: 'text-yellow-400', desc: 'Mất lượt', isStun: true, duration: 0 },
  VULNERABLE: { id: 'eff_vuln', name: 'Suy Yếu', type: 'DEBUFF', icon: BrokenShieldIcon, color: 'text-purple-400', desc: '+20% ST nhận vào', incomingPercent: 20, duration: 0 },
};
...
const StatAllocationRowDetailed = ({ label, code, val, tempVal, breakdownKey, icon, player, statBreakdown, expandedStat, setExpandedStat }) => {
  ...
  return (
    <div className="bg-slate-800 rounded border border-slate-700 overflow-hidden mb-2 transition-all">
      ...
      {isExpanded && bd && (
        <div className="bg-slate-900 p-2 text-xs space-y-1 border-t border-slate-700">
...
function Game() {
  ...
  const claimLoot = useCallback(() => {
    ...
    const newMaxFloor = Math.max(maxFloor, floor + 1);
    setMaxFloor(newMaxFloor);
    ...
    setGameState(GAME_STATE.MAP);
    setActiveModal(MODAL_STATE.NONE);
  }, [loot, player, stats.goldMult, addLog, addEffect, floor, maxFloor]);
  ...
  const handleMerge = useCallback((item1) => {
    if (!item1 || item1.type === 'CONSUMABLE') {
      addLog("Không thể hợp nhất vật phẩm này!", "text-red-500");
      return;
    }
        ...
        <div className="flex-1 relative overflow-hidden flex flex-col">
          ...
          {gameState === GAME_STATE.MAP && (
            <div className="h-full flex flex-col items-center justify-center p-4 bg-black/60 z-10">
              ...
              {maxFloor > 1 && (
                <button 
                  onClick={() => setActiveModal(MODAL_STATE.TRAVEL)}
                  className="mt-6 bg-purple-600 hover:bg-purple-700 px-6 py-2 rounded font-bold transition-colors flex items-center gap-2"
                >
                  <MapPin size={16}/> DI CHUYỂN
                </button>
              )}
            </div>
          )}
          ...
          {activeModal === MODAL_STATE.TRAVEL && (
            <div className="absolute inset-0 bg-slate-900/95 z-50 flex flex-col items-center justify-center p-6">
              <h2 className="text-2xl font-bold mb-6 text-center">DI CHUYỂN ĐẾN TẦNG</h2>
              <div className="bg-slate-800 p-6 rounded-lg w-full max-w-xs space-y-3 max-h-[400px] overflow-y-auto">
                {Array.from({ length: maxFloor }).map((_, i) => (
                  <button 
                    key={i}
                    onClick={() => {
                      setSelectedFloor(i + 1);
                      startFloor(i + 1, zone.id);
                      setActiveModal(MODAL_STATE.NONE);
                    }}
                    className={`w-full p-3 rounded border-2 font-bold transition-colors ${
                      floor === i + 1 
              ...
              <button 
                ...
                className="mt-6 bg-slate-700 hover:bg-slate-600 px-6 py-2 rounded text-white transition-colors"
              >
                Đóng
              </button>
            </div>
              ...
              {selectedItem && selectedItem.uid && (
                <div className="bg-slate-800 p-4 border-t border-slate-700">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    {selectedItem.type !== 'CONSUMABLE' && selectedItem.type !== 'SERVICE' && (
                      <button 
                    ...
                    {!selectedItem.isEquipped && selectedItem.type !== 'SERVICE' && (
                      <button 
                        onClick={() => {
                          setPlayer(p => ({ 
                            ...p, 
