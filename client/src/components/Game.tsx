...
function Game() {
  ...
  const enterRoom = useCallback((r) => {
    ...
    if (['COMBAT', 'ELITE', 'BOSS'].includes(r.type)) { setMonster(generateMonster(floor, r.type, zone, EFFECTS_DB)); setDiceResult(null); setGameState(GAME_STATE.COMBAT); }
  ...
  const handleCombat = useCallback((moveId) => {
    ...
    if (playerTick.delta !== 0) addEffect(`${playerTick.delta > 0 ? '+' : ''}${playerTick.delta}`, playerTick.delta > 0 ? 'heal' : 'damage', 68, 78);

    const nextPlayer = playerTick.entity;
    if (nextPlayer.hp <= 0) { setPlayer(nextPlayer); setGameState(GAME_STATE.GAME_OVER); return; }

    const combatResult = resolveRpsRound(moveId, null, currentRealStats, monster, nextPlayer, {
    ...
    combatResult.effectEvents.forEach((event) => addEffect(event.text, event.type, event.x, event.y));

    setHitState({ p: combatResult.damage.monsterToPlayer > 0, m: combatResult.damage.playerToMonster > 0 });
    setTimeout(() => setHitState({ p: false, m: false }), 200);

    setTimeout(() => {
      if (combatResult.monster.hp <= 0) { setLoot(generateLoot(zone.id, floor, monster.roomType || 'COMBAT', player.classId, currentRealStats.luck)); setGameState(GAME_STATE.VICTORY); addLog(`Đã đánh bại ${monster.name}!`, 'text-green-400'); return; }
      if (combatResult.player.hp <= 0) { setGameState(GAME_STATE.GAME_OVER); addLog('Bạn đã thua trận!', 'text-red-500'); return; }
      const monsterTick = resolveEffectTick(combatResult.monster, combatResult.monster.maxHp);
      if (monsterTick.delta !== 0) addEffect(`${monsterTick.delta > 0 ? '+' : ''}${monsterTick.delta}`, monsterTick.delta > 0 ? 'heal' : 'damage', 32, 24);
  ...
  }, [monster, gameState, player, currentRealStats, addLog, addEffect, zone.id, floor]);
