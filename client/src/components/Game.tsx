...
function Game() {
  ...
  const handleCombat = useCallback(
    (moveId) => {
      ...
      const playerTick = resolveEffectTick(player, currentRealStats.maxHp);
      if (playerTick.delta !== 0) {
        const tickEffect = createCombatEffect(
          `${playerTick.delta > 0 ? "+" : ""}${playerTick.delta}`,
          playerTick.delta > 0 ? "heal" : "damage",
          68,
          78,
        );
        addEffect(tickEffect.text, tickEffect.type, tickEffect.x, tickEffect.y);
      }
      ...
      const combatResult = resolveRpsRound(
        moveId,
        currentRealStats,
        monster,
        nextPlayer,
        {
          ...
          shieldEffectId: EFFECTS_DB.SHIELD.id,
          effectCatalog: EFFECTS_DB,
        },
      );

      setDiceResult(combatResult.diceResult);
      setPlayer(combatResult.player);
      setMonster(combatResult.monster);

      combatResult.logEvents.forEach((event) => addLog(event.msg, event.color));
      combatResult.effectEvents.forEach((event) =>
        addEffect(event.text, event.type, event.x, event.y),
      );

      setHitState({
        p: combatResult.damage.monsterToPlayer > 0,
        m: combatResult.damage.playerToMonster > 0,
      });
      setTimeout(() => setHitState({ p: false, m: false }), 200);

      setTimeout(() => {
        if (combatResult.monster.hp <= 0) {
          setLoot(
            generateLoot(
              zone.id,
              floor,
        ...
        if (combatResult.player.hp <= 0) {
          setGameState(GAME_STATE.GAME_OVER);
          addLog("Bạn đã thua trận!", "text-red-500");
          return;
        }

        const monsterTick = resolveEffectTick(
          combatResult.monster,
          combatResult.monster.maxHp,
        );
        if (monsterTick.delta !== 0) {
          addEffect(
            `${monsterTick.delta > 0 ? "+" : ""}${monsterTick.delta}`,
            monsterTick.delta > 0 ? "heal" : "damage",
            32,
            24,
          );
        }
