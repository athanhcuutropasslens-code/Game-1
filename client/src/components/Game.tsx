...
import {
  ...
  runTickEffects as runTickEffectsPure,
...
const ITEMS_DB = [
  // Healing & Utility
  {
    ...
    rarity: 1,
    baseVal: 30,
    descFormat: (val) => `Hồi ${val} HP`,
    effect: (p, ctx) => ({
      ...p,
      hp: Math.min(ctx.stats.maxHp, p.hp + ctx.value),
    }),
    icon: Heart,
  },
  {
  ...
  // Buffs
  {
    ...
    rarity: 2,
    baseVal: 5,
    descFormat: (val) => `+${val} ATK trong 3 lượt`,
    effect: (p, ctx) => applyStatusEffect(p, "STRONG_ATK", 3, ctx.value),
    icon: Sword,
  },
  {
    id: "scroll_shield",
    ...
    rarity: 3,
    baseVal: 20,
    descFormat: (val) => `Tạo lá chắn ${val} sát thương`,
    effect: (p, ctx) => applyStatusEffect(p, "SHIELD", 99, ctx.value),
    icon: Scroll,
  },
...
const findZone = (zoneId) =>
  ZONES_DB.find((z) => z.id === zoneId) || ZONES_DB[0];
...
const computeFullStats = (player, tempAlloc = null) => {
  ...
  const classMod =
    classId && CLASSES_DB[classId] ? CLASSES_DB[classId].baseMod : {};
...
const generateItem = (baseItem, level = 1, forceRarity = null) => {
  const item = {
    ...
    uid: randomId(),
    level,
    rarity: forceRarity || baseItem.rarity || rollRarity(level),
  };
...
const generateLoot = (zoneId, floor, roomType, playerClass, luck = 0) => {
  ...
  if (Math.random() < Math.min(1, itemChance + luck * 0.01)) {
    const usableItems = ITEMS_DB.filter((i) =>
      isItemUsableByClass(i, playerClass),
    );
    if (usableItems.length > 0)
      item = generateItem(
        usableItems[Math.floor(Math.random() * usableItems.length)],
        floor,
        itemRarity,
      );
  }
...
const generateShop = (floor, playerClass) => {
  const items = [
    generateItem(ITEMS_DB[0], 1, 1),
    generateItem(ITEMS_DB[1], 1, 2),
  ];
  const usableEquip = ITEMS_DB.filter(
    (i) =>
      !["CONSUMABLE", "SERVICE"].includes(i.type) &&
      isItemUsableByClass(i, playerClass),
  );
  ...
  if (weapons.length)
    items.push(
      generateItem(weapons[Math.floor(Math.random() * weapons.length)], floor),
    );
  if (armors.length)
    items.push(
      generateItem(armors[Math.floor(Math.random() * armors.length)], floor),
    );
  ...
  for (let i = 0; i < 2 + Math.floor(floor / 4); i++) {
    ...
    items.push(
      generateItem(
        base,
        Math.max(1, floor + Math.floor(Math.random() * 2)),
        Math.min(4, rollRarity(floor)),
      ),
    );
  }
  items.push({
...
const PixelAvatar = ({ seed, size = 100, type = "hero", isDead = false }) => {
  ...
  useEffect(() => {
    ...
    if (type === "hero") colors = ["#3b82f6", "#1d4ed8", "#93c5fd", "#fcd34d"];
      ...
      colors = ["#ef4444", "#7f1d1d", "#10b981", "#7c3aed", "#f59e0b"];
    else if (type === "boss")
      colors = ["#7c3aed", "#4c1d95", "#c4b5fd", "#ffffff"];
    ...
    for (let y = 0; y < gridSize; y++) {
      for (let x = 0; x < gridSize / 2; x++) {
        if (random(seed + x * y * 13) > 0.5) {
          const color =
            colors[Math.floor(random(seed + x + y) * colors.length)];
          ctx.fillStyle = isDead ? "#4b5563" : color;
          ctx.fillRect(x * pixelSize, y * pixelSize, pixelSize, pixelSize);
          ctx.fillRect(
            (gridSize - 1 - x) * pixelSize,
            y * pixelSize,
            pixelSize,
            pixelSize,
          );
...
const SpriteSheetAvatar = ({
  ...
  useEffect(() => {
    ...
    const drawPx = (x, y, color) => {
      ...
      ctx.fillRect(
        (x + offset) * (canvas.width / frameSize),
        y * (canvas.height / frameSize),
    ...
    if (variant === "hero") {
      ...
      (weaponPose[id] || weaponPose.warrior).forEach(([x, y]) =>
        drawPx(x, y, pal.weapon),
      );
    } else {
      ...
      (shape[id] || shape.slime).forEach(([x, y], idx) =>
        drawPx(x, y + (activeFrame === 1 ? -1 : 0), pal[idx % pal.length]),
      );
      drawPx(6, 6, "#111827");
      drawPx(8, 6, "#111827");
      if (tier !== "normal") {
...
const CombatAvatar = ({
  ...
  const tier =
    roomType === "BOSS" ? "boss" : roomType === "ELITE" ? "elite" : "normal";
  const useSheet =
    SPRITE_MODE === "sheet" &&
  ...
  if (!useSheet) {
    return (
      <PixelAvatar
        ...
        size={size}
        type={
          role === "hero" ? "hero" : roomType === "BOSS" ? "boss" : "monster"
        }
        isDead={isDead}
      />
    );
...
const StatAllocationRowDetailed = ({
  label,
  code,
  val,
  tempVal,
  ...
  const diff =
    tempVal !== undefined && player ? tempVal - player.statsAllocated[code] : 0;
  const finalVal =
    val +
    (code === "def"
      ? Math.floor(diff * 0.5)
  ...
  return (
    <div className="bg-slate-800 rounded border border-slate-700 overflow-hidden mb-2 transition-all">
      <div
        ...
        onClick={() => setExpandedStat(isExpanded ? null : breakdownKey)}
      >
        ...
        <div className="flex items-center gap-2">
          <span
            className={`font-bold ${diff > 0 ? "text-green-400" : "text-white"}`}
          >
            {finalVal}
          </span>
          {diff > 0 && (
...
function Game() {
  ...
  const { final: stats, breakdown: statBreakdown } = useMemo(
    () => computeFullStats(player, tempStats),
    [player, tempStats],
  );
  const { final: currentRealStats } = useMemo(
    () => computeFullStats(player),
    [player],
  );
  const allSelectableItems = useMemo(
    () => [
      ...
      ...Object.entries(player.equipment)
        .filter(([, item]) => item)
        .map(([slot, item]) => ({ ...item, isEquipped: true, slot })),
    ],
    [player.inventory, player.equipment],
  );
  const selectedItem = useMemo(
    () => allSelectableItems.find((item) => item.uid === selectionUID) || null,
    [allSelectableItems, selectionUID],
  );
  ...
  // --- GAME ACTIONS ---
  const runTickEffects = useCallback(
    (entity, isPlayer = false, maxHpOverride = null) => {
      const maxHp =
        maxHpOverride ?? (isPlayer ? currentRealStats.maxHp : entity.maxHp);
      const {
        entity: processed,
        isStunned,
        deltas,
      } = runTickEffectsPure({ entity, maxHp });
      deltas.forEach(({ amount }) => {
        if (amount !== 0)
          addEffect(
            `${amount > 0 ? "+" : ""}${amount}`,
  ...
  const selectClass = useCallback(
    (cId) => {
      ...
      const starterArmId =
        cId === "warrior"
          ...
          : cId === "mage" || cId === "cleric"
            ...
            : "a_leather";
      const weaponBase =
        ITEMS_DB.find((i) => i.id === starterWepId) || ITEMS_DB[5];
      const armorBase =
        ITEMS_DB.find((i) => i.id === starterArmId) || ITEMS_DB[9];
      let nextPlayer = {
        ...player,
        classId: cId,
        statPoints: 5,
      ...
      if (cId === "warrior")
        nextPlayer = applyStatusEffect(nextPlayer, "SHIELD", 99, 10);
  ...
  const startFloor = useCallback(
    (f, zId) => {
      ...
      addLog(
        `Tầng ${f} - ${findZone(zId).name}${isCleared ? " (đã dọn)" : ""}`,
      );
    },
  ...
  const completeRoom = useCallback(
    (
      ...
      roomType = mapRooms.find((r) => r.id === roomId)?.type,
    ) => {
      const isBoss = roomType === "BOSS";
      setMapRooms((prev) =>
        prev.map((room) => {
          if (room.id === roomId)
            return { ...room, completed: true, locked: false };
          if (!isBoss && room.id === roomId + 1)
      ...
      if (isBoss) {
        ...
        setPlayer((p) => ({
          ...p,
          maxFloor: Math.max(p.maxFloor || 1, nextFloor),
        }));
        setFloor(nextFloor);
        setTimeout(() => startFloor(nextFloor, zone.id), 250);
        addLog(
          `Đã chinh phục tầng ${floor}. Mở khóa tầng ${nextFloor}!`,
          "text-yellow-400",
        );
      }
    },
  ...
  const closeShop = useCallback(() => {
    completeRoom(currentRoomId, "SHOP");
  ...
  const enterRoom = useCallback(
    (r) => {
      if (r.locked)
        return addLog("Phòng này vẫn đang bị khóa.", "text-red-400");
      setCurrentRoomId(r.id);
      if (["COMBAT", "ELITE", "BOSS"].includes(r.type) && r.completed) {
  ...
  const handleCombat = useCallback(
    (moveId) => {
      ...
      const resolveMonsterAttack = (baseDamage) => {
        const amplifiedDamage = calculateIncomingDamage(nextPlayer, baseDamage);
        const mitigated = mitigateIncomingDamage(nextPlayer, amplifiedDamage);
        nextPlayer = {
          ...mitigated.entity,
          hp: Math.max(0, mitigated.entity.hp - mitigated.damageTaken),
        };
        if (mitigated.absorbed > 0)
          addEffect(`BLOCK ${mitigated.absorbed}`, "heal", 68, 70);
        return mitigated.damageTaken;
      };

      if (!playerTick.isStunned) {
        const attackMult =
          outcome === "win" ? 1.35 : outcome === "lose" ? 0.8 : 1;
        const defendMult =
          outcome === "lose" ? 1.2 : outcome === "win" ? 0.85 : 1;
        const crit = Math.random() * 100 < currentRealStats.crit;
        playerDamage = Math.floor(
          (Math.floor(Math.random() * currentRealStats.diceSides) +
            1 +
            currentRealStats.atk) *
            attackMult *
            (crit ? 1.5 : 1),
        );
        const baseMonsterDamage = Math.max(
          0,
          Math.floor(
            (Math.floor(Math.random() * monster.diceSides) +
              1 +
              monster.atk -
              currentRealStats.def) *
              defendMult,
          ),
        );
        monsterDamage = resolveMonsterAttack(baseMonsterDamage);
        nextMonster = {
          ...monster,
          hp: Math.max(0, monster.hp - playerDamage),
        };
        if (player.classId === "rogue" && Math.random() < 0.2)
          nextMonster = applyStatusEffect(nextMonster, "POISON", 3);
        if (player.classId === "mage" && Math.random() < 0.2)
          nextMonster = applyStatusEffect(
            nextMonster,
            ["POISON", "STUN", "VULNERABLE"][Math.floor(Math.random() * 3)],
            2,
          );
        addLog(
          `${move.name} vs ${monsterMove.name}: ${outcome === "win" ? "thắng thế" : outcome === "lose" ? "lép vế" : "hòa"}.`,
          outcome === "win"
            ? "text-green-400"
            : outcome === "lose"
              ? "text-red-400"
      ...
      setTimeout(() => {
        ...
        const monsterTick = runTickEffects(
          nextMonster,
          false,
          nextMonster.maxHp,
        );
        setMonster(monsterTick.entity);
        if (monsterTick.entity.hp <= 0) {
          setLoot(
            generateLoot(
              zone.id,
              floor,
  ...
  const claimLoot = useCallback(() => {
    ...
    setPlayer((p) => ({
      ...
      skillPoints,
      baseStats,
      hp: clamp(hp, 0, computeFullStats({ ...p, baseStats }).final.maxHp),
      inventory: loot.item ? [...p.inventory, loot.item] : p.inventory,
    }));
    ...
    completeRoom(
      currentRoomId,
      mapRooms.find((r) => r.id === currentRoomId)?.type,
    );
  }, [
    loot,
    player,
  ...
  const adjustStat = useCallback(
    (key, delta) => {
      if (delta > 0 && tempStatPoints > 0) {
        ...
        delta < 0 &&
        tempStats[key] > (player.statsAllocated[key] || 0)
      ) {
        setTempStats((p) => ({ ...p, [key]: p[key] - 1 }));
        setTempStatPoints((p) => p + 1);
      }
  ...
  const commitStats = useCallback(() => {
    setPlayer((p) => ({
      ...
      hp: Math.min(
        p.hp,
        computeFullStats({ ...p, statsAllocated: tempStats }).final.maxHp,
      ),
    }));
    setActiveModal(MODAL_STATE.NONE);
  ...
  const upgradeSkill = useCallback(
    (skillId) => {
      ...
      const skill = CLASSES_DB[player.classId]?.skills.find(
        (s) => s.id === skillId,
      );
      if (!skill || currentLevel >= skill.max || player.skillPoints <= 0)
        return;
      setPlayer((p) => ({
        ...p,
  ...
  const useConsumable = useCallback(
    (item) => {
      ...
      const value = getConsumableValue(item, player.classId);
      setPlayer((prev) => ({
        ...item.effect(prev, { stats: currentRealStats, value }),
        inventory: prev.inventory.filter((i) => i.uid !== item.uid),
      }));
      setSelectionUID(null);
      addLog(`Đã dùng ${item.name}`, "text-green-400");
      addEffect(
        item.id.startsWith("pot_") ? `+${value}` : "BUFF",
        "heal",
        68,
        74,
      );
    },
    [player.classId, currentRealStats, addLog, addEffect],
  );

  const equipItem = useCallback(
    (item) => {
      if (!player.classId) return;
      if (!isItemUsableByClass(item, player.classId))
        return addLog(
          "Lớp này không thể trang bị vật phẩm này!",
          "text-red-500",
        );
      const slot =
        item.type === "WEAPON"
  ...
  const unequipItem = useCallback(
    ...
    [player.equipment, addLog],
  );

  const handleMerge = useCallback(
    (item1) => {
      const sameTypeMatcher = (i) =>
        item1.type === "CONSUMABLE"
          ? i.id === item1.id && i.rarity === item1.rarity
          : i.id === item1.id && i.level === item1.level;
      const invMatch = player.inventory.find(
        (i) => sameTypeMatcher(i) && i.uid !== item1.uid,
      );
      const slot =
        item1.type === "WEAPON"
          ? "weapon"
          : item1.type === "ARMOR"
            ? "armor"
            : item1.type === "ACCESSORY"
              ? "accessory"
              : null;
      const equipMatch =
        slot &&
        player.equipment[slot] &&
        sameTypeMatcher(player.equipment[slot]) &&
        player.equipment[slot].uid !== item1.uid
          ? player.equipment[slot]
          : null;
      const matchItem = invMatch || equipMatch;
      if (!matchItem)
        return addLog("Không có vật phẩm phù hợp để hợp nhất!", "text-red-500");
      const merged = generateItem(
        ITEMS_DB.find((base) => base.id === item1.id) || item1,
        (item1.level || 1) + 1,
        item1.type === "CONSUMABLE"
          ? Math.min(5, (item1.rarity || 1) + 1)
          : Math.max(item1.rarity || 1, matchItem.rarity || 1),
      );
      merged.affixes = [
        ...new Map(
          [...(item1.affixes || []), ...(matchItem.affixes || [])].map((a) => [
            a.id,
            a,
          ]),
        ).values(),
      ];
      merged.cost = calculateCost(merged);
      merged.sellPrice = Math.floor(merged.cost * 0.45);
      const newInv = player.inventory.filter(
        (i) => i.uid !== item1.uid && i.uid !== matchItem.uid,
      );
      const newEquip = { ...player.equipment };
      const shouldEquip = slot && (item1.isEquipped || false || !!equipMatch);
      if (slot && shouldEquip) newEquip[slot] = merged;
      else newInv.push(merged);
      setPlayer((p) => ({ ...p, inventory: newInv, equipment: newEquip }));
      setSelectionUID(merged.uid);
      addLog(
        `Hợp nhất thành công: ${merged.name} +${merged.level}`,
        "text-purple-400",
      );
      addEffect("MERGE!", "heal", 50, 50);
    },
  ...
  const buyService = useCallback(
    (service) => {
      if (player.gold < service.cost)
        return addLog("Không đủ vàng!", "text-red-500");
      if (service.id === "srv_heal") {
        setPlayer((p) => ({
          ...p,
          gold: p.gold - service.cost,
          hp: currentRealStats.maxHp,
        }));
        return addLog("Đã hồi phục toàn bộ HP!", "text-green-400");
      }
      if (service.id === "srv_box") {
        const rarity = 2 + Math.floor(Math.random() * 3);
        const usable = ITEMS_DB.filter((i) =>
          isItemUsableByClass(i, player.classId),
        );
        ...
        addLog(
          `Nhận được ${newItem.name} (${RARITY_CONFIG[rarity].name})`,
          "text-yellow-400",
        );
      }
    },
    [player.gold, player.classId, currentRealStats.maxHp, floor, addLog],
  );
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white font-mono select-none flex items-center justify-center p-2">
      <div
        className={`w-full max-w-[400px] h-[800px] bg-gradient-to-b ${zone.bg} border-4 border-slate-700 rounded-2xl overflow-hidden shadow-2xl relative flex flex-col`}
      >
        {/* HEADER */}
        <div className="h-14 bg-slate-900/90 backdrop-blur border-b-4 border-slate-700 flex items-center justify-between px-3 z-20 shrink-0">
          <div className="flex flex-col">
            <div className="flex items-center gap-2 text-xs font-bold">
              <span className="bg-yellow-600 px-1.5 rounded text-black">
                LV.{player.level}
              </span>
              <div className="w-20 h-2 bg-slate-900 rounded-full border border-slate-600 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, (player.exp / player.nextLevelExp) * 100)}%`,
                  }}
                ></div>
              </div>
            </div>
        ...
        <div className="flex-1 relative overflow-hidden flex flex-col">
          ...
          {gameState === GAME_STATE.MENU && (
            <div className="h-full flex flex-col items-center justify-center gap-6 bg-black/80 z-10 p-4">
              <h1 className="text-4xl font-black text-blue-500 text-center">
                PIXEL ROGUE
              </h1>
              <div className="text-slate-400 text-sm text-center max-w-xs">
                Game nhập vai chiến đấu theo lượt với hệ thống vật phẩm và nâng
                cấp phong phú
              </div>
              <button
                onClick={initGame}
            ...
            <div className="h-full p-4 overflow-y-auto bg-black/80 z-10">
              ...
              <div className="space-y-3">
                {Object.values(CLASSES_DB).map((c) => (
                  ...
                  >
                    <div
                      className={`font-bold ${c.color} flex items-center gap-2`}
                    >
          ...
          {gameState === GAME_STATE.MAP && (
            <div className="h-full flex flex-col items-center justify-center p-4 bg-black/60 z-10">
              ...
              <div className="mb-4 text-center text-xs text-slate-400 max-w-xs">
                {zone.desc}
              </div>
              <div className="mb-4 grid grid-cols-2 gap-2 w-full max-w-xs text-[10px]">
                <div className="rounded border border-slate-700 bg-black/30 p-2 text-slate-300">
                  May mắn:{" "}
              ...
              <div className="flex flex-col gap-3 w-full max-w-xs">
                {mapRooms.map((r) => (
                  <button
                    key={r.id}
                    disabled={
                      r.locked || (r.id !== 0 && !mapRooms[r.id - 1]?.completed)
                    }
                    onClick={() => enterRoom(r)}
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="flex items-center gap-2">
                        {renderIcon(
                          ROOM_TYPES[r.type].icon,
                          14,
                          ROOM_TYPES[r.type].color,
                        )}
                        {ROOM_TYPES[r.type].label}
                      </span>
              ...
              <div className="h-full flex flex-col p-4">
                ...
                <div className="mt-4 flex flex-col items-center">
                  ...
                  <div className="mt-2 flex flex-wrap justify-center gap-1">
                    {(monster.effects || []).map((eff) => (
                      <span
                        key={eff.uid}
                        className={`rounded border border-slate-700 bg-slate-900 px-2 py-1 text-[10px] ${eff.color}`}
                      >
                        {eff.name} {eff.value ? `(${eff.value})` : ""}
                      </span>
                    ))}
                  </div>
                </div>
                  ...
                  {diceResult && (
                    <div className="rounded border border-slate-700 bg-black/50 px-3 py-2 text-center text-xs">
                      <div className="text-slate-300">
                        {diceResult.pMove.icon} {diceResult.pMove.name} vs{" "}
                        {diceResult.mMove.icon} {diceResult.mMove.name}
                      </div>
                      <div
                        className={`font-bold ${diceResult.outcome === "win" ? "text-green-400" : diceResult.outcome === "lose" ? "text-red-400" : "text-yellow-400"}`}
                      >
                        {diceResult.outcome === "win"
                          ? "Bạn chiếm ưu thế"
                          : diceResult.outcome === "lose"
                            ? "Quái phản công mạnh hơn"
                            : "Hòa thế"}
                      </div>
                    </div>
                  )}
                ...
                <div className="mb-4 flex flex-col items-center">
                  <div className="w-48 bg-slate-900 h-4 rounded-full border border-slate-600 overflow-hidden mb-2 relative">
                    <div
                      ...
                      style={{
                        width: `${Math.max(0, (player.hp / stats.maxHp) * 100)}%`,
                      }}
                    ></div>
                        ...
                        >
                          ...
                          <div className="flex gap-3 items-center">
                            <button
                              ...
                              disabled={
                                tempStats[s.c] <=
                                (player.statsAllocated[s.c] || 0)
                              }
                            >
                              <Minus size={12} />
                            </button>
                            <span
                              className={`w-6 text-center font-bold ${tempStats[s.c] > (player.statsAllocated[s.c] || 0) ? "text-green-400" : "text-white"}`}
                            >
                              {tempStats[s.c]}
                            </span>
                            <button
            ...
            <div className="absolute inset-0 bg-slate-900/95 z-50 flex flex-col">
              <div className="p-4 border-b border-slate-800 flex items-center justify-between">
                <div className="font-bold flex items-center gap-2">
                  <Footprints size={16} /> Di Chuyển Tầng
                </div>
                <button onClick={() => setActiveModal(MODAL_STATE.NONE)}>
                  <X />
                </button>
              ...
              <div className="p-4 grid grid-cols-2 gap-2">
                {Array.from(
                  { length: player.maxFloor || 1 },
                  (_, idx) => idx + 1,
                ).map((f) => (
                  <button
                    key={f}
                    onClick={() => travelToFloor(f)}
                    className={`rounded border p-3 text-sm font-bold ${f === floor ? "border-yellow-500 bg-slate-700" : "border-slate-700 bg-slate-800 hover:border-cyan-400"}`}
                  >
            ...
            <div className="absolute inset-0 bg-slate-900 z-50 flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto p-4">
                    ...
                    {["weapon", "armor", "accessory"].map((slot) => (
                      <div key={slot} className="flex flex-col items-center">
                        ...
                        {player.equipment[slot] ? (
                          <div
                            onClick={() =>
                              setSelectionUID(player.equipment[slot].uid)
                            }
                          ...
                          >
                            <PixelItemIcon
                              ...
                              fallbackIcon={player.equipment[slot].icon}
                              size={24}
                            />
                          </div>
                        ) : (
                ...
                {player.inventory.length > 0 && (
                  <div className="flex justify-between items-center mb-4 px-1">
                    ...
                    <span className="text-xs text-slate-400">
                      ...
                      {player.inventory.length} vật phẩm)
                    </span>
                    <button
                ...
                <div className="grid grid-cols-4 gap-2 min-h-[200px] content-start">
                  ...
                  {Array.from({
                    length: Math.max(
                      0,
                      ITEMS_PER_PAGE - currentInventoryPage.length,
                    ),
                  }).map((_, i) => (
                    <div
                      key={`empty-${i}`}
              ...
              {selectedItem && selectedItem.uid && (
                <div className="bg-slate-800 p-4 border-t border-slate-700">
                  <div
                    className={`font-bold text-lg mb-1 ${RARITY_CONFIG[selectedItem.rarity].color.replace("border-", "text-")}`}
                  >
                    {selectedItem.name}{" "}
                    {selectedItem.level ? `+${selectedItem.level}` : ""}
                  </div>
                  ...
                  {selectedItem.type !== "CONSUMABLE" && (
                    <div className="bg-slate-900 p-2 rounded mb-3 text-xs space-y-1">
                      {Object.entries(getItemStats(selectedItem)).map(
                        ([k, v]) => (
                          <div
                            key={k}
                          ...
                          >
                            ...
                            <span className="text-white font-bold">
                              ...
                              <span className="text-slate-500 font-normal ml-1">
                                ({selectedItem.baseStats?.[k] || 0} gốc +{" "}
                                {v - (selectedItem.baseStats?.[k] || 0)} cấp)
                              </span>
                            </span>
                          </div>
                        ),
                  ...
                  <div className="text-xs text-yellow-500 mb-3">
                    {selectedItem.type === "CONSUMABLE"
                      ? selectedItem.descFormat
                        ? selectedItem.descFormat(
                            getConsumableValue(selectedItem, player.classId),
                          )
                        : selectedItem.desc
                      : selectedItem.desc ||
                        "Trang bị hiếm dùng để gia tăng sức mạnh."}
                  </div>
                  {selectedItem.affixes?.length > 0 && (
                    <div className="mb-3 flex flex-wrap gap-1">
                      {selectedItem.affixes.map((affix) => (
                        <span
                          key={affix.id}
                          className="rounded bg-slate-900 px-2 py-1 text-[10px] text-cyan-300"
                        >
                          {affix.name} +{affix.val}
                        </span>
                      ))}
                    </div>
                  )}
                        ...
                        className={`py-2 rounded font-bold transition-colors ${
                          player.inventory.some(
                            (i) =>
                              ...
                              i.level === selectedItem.level &&
                              i.uid !== selectedItem.uid,
                          ) ||
                ...
                {shopItems.map((item) => (
                  <div
                    ...
                    className={`bg-slate-800 p-3 rounded border flex gap-3 items-center ${
                      item.type === "SERVICE"
                        ? "border-blue-500"
                        : RARITY_CONFIG[item.rarity || 1].color.replace(
                            "text-",
                            "border-",
                          )
                    }`}
              ...
              <div className="bg-slate-800 p-6 rounded-lg w-full max-w-xs space-y-4">
                <div className="flex justify-between font-bold text-white">
                  ...
                  <span className="text-yellow-400">
                    +
                    {Math.floor(
                      ((loot.gold || 0) * (stats.goldMult || 100)) / 100,
                    )}
                  </span>
                </div>
                <div className="flex justify-between font-bold text-white">
                ...
                {loot.item && (
                  <div className="mt-4 pt-4 border-t border-slate-600 text-center">
                    ...
                    <div
                      className={`font-bold ${RARITY_CONFIG[loot.item.rarity].color}`}
                    >
                      {loot.item.name}{" "}
                      {loot.item.level ? `+${loot.item.level}` : ""}
                    </div>
                  </div>
                )}
              </div>
